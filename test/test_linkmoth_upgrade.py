...
conn.execute("""CREATE TABLE load_tests(
  ...
  budget_limited INTEGER DEFAULT 0, loaded_loss_pct REAL)""")
...
with core.db() as c:
    ...
    row = c.execute("SELECT grade, loaded_loss_pct, loaded_local_loss_pct"
    ...
    c.execute("INSERT INTO load_tests(ts,idle_ms,loaded_ms,bloat_ms,grade,"
              ...
              "budget_limited,loaded_loss_pct,loaded_local_loss_pct)"
              " VALUES(2,7,9,2,'A',500,100,1,NULL,0.31,1,25.0,50.0)")
    fresh = c.execute("SELECT loaded_loss_pct, loaded_local_loss_pct"
                      " FROM load_tests ORDER BY id DESC LIMIT 1").fetchone()
...
class LoadTestsColumnMigrationTests(unittest.TestCase):
    ...
    def test_a_previous_release_database_gains_the_new_columns(self):
        ...
        self.assertIn("loaded_local_loss_pct", out["COLS"])
        ...
        self.assertEqual(out["NEW"], "(25.0, 50.0)")
