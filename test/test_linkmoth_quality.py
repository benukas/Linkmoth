...
class LoadTestTests(unittest.TestCase):
    ...
    def test_run_load_test_grades_and_stores(self):
        ...
        with mock.patch.object(
            self.lm, "measure_quality", return_value=idle,
        ), mock.patch.object(
            self.lm, "_measure_loaded_quality", return_value=loaded,
        ), mock.patch.object(
    ...
    def test_run_load_test_none_when_too_few_active_samples(self):
        ...
        with mock.patch.object(
            ...
            self.lm, "_measure_loaded_quality", return_value=None,
        ), mock.patch.object(
            self.lm, "_load_downloader", side_effect=fake_downloader,
        ), mock.patch.object(
            self.lm, "_resolve_load_target",
    ...
    def test_downloader_repeats_requests_within_a_shared_total_cap(self):
        ...
        stats = {"bytes": 0, "elapsed": 0.0, "error": None}
        with mock.patch.object(
            self.lm, "_PinnedHTTPSConnection", side_effect=[conn1, conn2],
        ) as pinned:
            self.lm._load_downloader(
                "https://speed.example.com/file",
    ...
    def test_measure_loaded_quality_counts_only_overlapping_samples(self):
        ...
        with mock.patch.object(
            self.lm, "measure_quality", side_effect=fake_measure,
        ), mock.patch.object(self.lm.time, "sleep"):
