...
```bash
VERSION=v0.6.7; NAME="linkmoth-$VERSION-bootstrap.sh"; curl -fsSLo "$NAME" --proto '=https' --noproxy '*' --max-redirs 0 "https://raw.githubusercontent.com/benukas/Linkmoth/$VERSION/bootstrap.sh" && sudo bash "$NAME"
```
...
```bash
VERSION=v0.6.7   # use your installed version (shown in the dashboard footer)
